...
data_path <- "H:/Biomass/Publication/Review/Data/"
figures_path <- "H:/Biomass/Publication/Review/Figures/"

biomass_data <- read.csv(file.path(data_path,"biomass_data_2024.csv"),header=T,sep=";")
...
png(filename = file.path(figures_path,"Figure_S2_Height_distribution.png"),
...
```{r, warning=FALSE}
png(filename = file.path(figures_path,"Figure_S5.1_Correlation_plot.png"),
    width = 30, height = 15, units = "cm",bg = "white", res = 300)
...
png(filename = file.path(figures_path,"Figure_S5.2_Dissimilarity_tree.png"),
    width = 30, height = 15, units = "cm",bg = "white", res = 300)
...
### Morans I of residuals to check if spatial autocorrelation is accounted for

residuals_gam <- residuals(model_total)
coords <- cbind(biomass_data$Lon, biomass_data$Lat)

model_data <- model.frame(model_total)  
used_rows <- as.numeric(rownames(model_data))
coords_used <- coords[used_rows, ]

k_values <- 1:15   # adjust as needed
...
for(i in seq_along(k_values)){
  k <- k_values[i]
  knn <- knearneigh(coords_used, k = k)
  nb <- knn2nb(knn)
  lw <- nb2listw(nb, style = "W", zero.policy = TRUE)
  moran_test <- moran.test(residuals_gam, lw, zero.policy = TRUE)
  moran_results[i] <- moran_test$estimate["Moran I statistic"]
}

# Combine results in a data frame for easy viewing
moran_df_total <- data.frame(k = k_values, Moran_I_total = moran_results)
print(moran_df_total)
...
residuals_gam <- residuals(model_shrub)
coords <- cbind(biomass_data$Lon, biomass_data$Lat)

model_data <- model.frame(model_shrub)  
used_rows <- as.numeric(rownames(model_data))
coords_used <- coords[used_rows, ]
...
# Combine results in a data frame for easy viewing
moran_df_shrub <- data.frame(k = k_values, Moran_I_shrub = moran_results)
print(moran_df_shrub)
...
residuals_gam <- residuals(model_forb)
coords <- cbind(biomass_data$Lon, biomass_data$Lat)

model_data <- model.frame(model_forb)  
used_rows <- as.numeric(rownames(model_data))
coords_used <- coords[used_rows, ]
...
# Combine results in a data frame for easy viewing
moran_df_forb <- data.frame(k = k_values, Moran_I_forb = moran_results)
print(moran_df_forb)
...
residuals_gam <- residuals(model_grass)
coords <- cbind(biomass_data$Lon, biomass_data$Lat)

model_data <- model.frame(model_grass)  
used_rows <- as.numeric(rownames(model_data))
coords_used <- coords[used_rows, ]
...
# Combine results in a data frame for easy viewing
moran_df_grass <- data.frame(k = k_values, Moran_I_grass = moran_results)
print(moran_df_grass)
...
residuals_gam <- residuals(model_herbs)
coords <- cbind(biomass_data$Lon, biomass_data$Lat)

model_data <- model.frame(model_herbs)  
used_rows <- as.numeric(rownames(model_data))
coords_used <- coords[used_rows, ]
...
# Combine results in a data frame for easy viewing
moran_df_herbs <- data.frame(k = k_values, Moran_I_herbs = moran_results)
print(moran_df_herbs)

dfs <- list(moran_df_total, moran_df_shrub, moran_df_forb, moran_df_grass, moran_df_herbs)

morans_full <- round(reduce(dfs, full_join, by = "k"),2)
print(morans_full)
...
png(filename = file.path(figures_path,"Figure_2_Variable_importance.png"),
...
png(filename = file.path(figures_path,"Figure_S8_Variable_importance_forb_grass.png"),
    width = 30, height = 30, units = "cm",bg = "white", res = 300)
...
png(filename = file.path(figures_path,"Figure_3_Cross_validation_results.png"),
    width = 30, height = 30, units = "cm",bg = "white", res = 300)
grid.arrange(total_plot, shrub_plot, herb_plot, layout_matrix = rbind(c(1,1), c(2,3)))
dev.off()

png(filename = file.path(figures_path,"Figure_S8_Cross_validation_results_forb_grass.png"),
    width = 30, height = 15, units = "cm",bg = "white", res = 300)
...
grid_data <- read.csv(file.path(data_path,"grid_data.csv"),header=T,sep=";")
...
```{r, warning=FALSE}
# Save results 
write.table(grid_data,file.path(data_path,"Grid_predict.csv"),row.names=F,sep=",")
...
png(filename = file.path(figures_path,"Figure_S10.1_Observed_pred_biomass_distribution.png"),
...
png(filename = file.path(figures_path,"Figure_S10.2_Pred_biomass_CV_distribution.png"),
    width = 30, height = 45, units = "cm",bg = "white", res = 300)
print(grid.arrange(total_3, total_4, shrub_3, shrub_4, herb_3, herb_4, forb_3, forb_4, grass_3, grass_4, nrow=5,widths=c(1/2,1/2)))
dev.off()

png(filename = file.path(figures_path,"Figure_S10.3_Historgramms_HAB_distribution.png"),
    width = 30, height = 45, units = "cm",bg = "white", res = 300)
